...
function SeverityBadge({ severity }: { severity: AttackSeverity }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium ${cfg.text} bg-base-800/60`}
    >
      <span className={`inline-flex size-1.5 rounded-full ${cfg.dot}`} />
      {cfg.label}
    </span>
  );
}
...
function LoadingCard() {
  return (
    <li className="rounded-lg border border-line bg-base-900 p-4">
    ...
    return (
      <li className="rounded-lg border border-line bg-base-900 p-4">
        ...
        <button
          ...
          className="mt-4 inline-flex h-8 items-center rounded-md border border-line px-3 text-xs font-medium text-ink-600 transition-colors hover:bg-base-800 active:scale-[0.98]"
        >
          Tentar novamente
        </button>
      </li>
  ...
  return (
    <li className="rounded-lg border border-line bg-base-900 p-4">
      ...
      <div className="mt-4 flex items-start gap-2 border-t border-line pt-3">
        <span className="text-xs font-medium text-ink-500">Veredito:</span>
        <span
      ...
      {isSuccess && result.mitigation.trim() !== "" && (
        <div className="mt-3 flex items-start gap-2 rounded-md border border-line bg-base-800/40 px-3 py-2">
          <span className="text-xs font-medium text-ink-500">Mitigacao:</span>
    ...
    <main className="mx-auto w-full max-w-[1400px] px-page py-section lg:px-page-lg lg:py-section-lg">
      <div className="flex items-center justify-between gap-4">
        ...
        <button
          type="button"
          onClick={onNewAudit}
          className="inline-flex h-9 items-center rounded-md border border-line px-4 text-sm font-medium text-ink-600 transition-colors hover:bg-base-850 active:scale-[0.98]"
        >
          Nova auditoria
        </button>
      </div>
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-12">
        <aside className="flex flex-col gap-6 lg:col-span-4">
          <section
            ...
            className="rounded-lg border border-line bg-base-850/60 p-4"
          >
            <h2 className="text-xs font-semibold uppercase tracking-tag text-ink-500">
              System prompt
            </h2>
            <div className="mt-3 max-h-52 overflow-y-auto rounded-md border border-line bg-base-900 px-3 py-2.5 font-mono text-xs leading-relaxed text-ink-500">
              {systemPrompt.trim() !== ""
                ? systemPrompt
        ...
        <div className="flex flex-col gap-6 lg:col-span-8">
          <section aria-label="Cartoes de ataque" className="flex flex-col gap-3">
            <h2 className="text-xs font-semibold uppercase tracking-tag text-ink-500">
            ...
            <ul className="flex flex-col gap-3">
              {cards.map((c) =>
                c.kind === "loading" ? (
                  ...
                  <li
                    key={c.id}
                    className="rounded-lg border border-line bg-base-900 p-4"
                  >
